...
def get_qq_access_token():
    """获取 QQ 机器人 Access Token（Token 鉴权已废弃）"""
    if not QQ_BOT_APP_ID or not QQ_BOT_APP_SECRET:
        return None
    try:
        res = requests.post(
            QQ_BOT_TOKEN_URL,
            json={"appId": QQ_BOT_APP_ID, "clientSecret": QQ_BOT_APP_SECRET},
            timeout=10
        )
        res.raise_for_status()
        token = res.json().get("access_token")
        if not token:
            print(f"❌ QQ Token 获取失败: {res.text}")
        return token
    except Exception as e:
...
def push_all(title, body, markdown, image_url):
    """执行多通道推送"""
    push_qq_message(title, body, image_url)
